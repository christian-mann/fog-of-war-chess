...
def leak(f):
	def leakyF(*args, **kwargs):
		print "%s(%s)" % (f.__name__, ', '.join([str(a) for a in args]))
		f(*args, **kwargs)
	return leakyF
	
class World(DirectObject):
	def __init__(self, mode=SERVER, ip=None):
		...
		self.accept('mouse2', self.makeRandomMove)
		self.accept('mouse3', self.showVisibleSquares)
	...
	def setupClient(self, ip):
		...
		if myConnection:
			...
			print "Testing connection..."
			while not self.cReader.dataAvailable():
				pass
			print "Receiving nonce..."
			datagram = NetDatagram()
			if self.cReader.getData(datagram):
				print "Nonce: %d " % PyDatagramIterator(datagram).getUint8()
	...
	def showSquare(self, sq, dt="default", callback=None):
		...
		if self.squares[sq] and self.squares[sq].getPos().z < 0:
			...
			dest = self.squares[sq].getPos()
			if dt == "default": dt = abs(dest.z/10)
			dest.z = 0
	...
	def rotateCamera(self):
		#to rotate the camera around the board, a few things need to happen at the same time.
		dt = 2
		pos = camera.getPos()
		#the camera's yaw needs to be changed by 180
		dh = camera.hprInterval(dt, camera.getHpr() + Point3(180,0,0))
		#the camera's position should be flipped in the Y direction
		y = pos.y
		dy = camera.posInterval(dt, pos + Point3(0,-2*y,0))
		
		dx = Sequence(
			camera.posInterval(dt/2, pos + Point3(10,0,0)),
			camera.posInterval(dt/2, pos)
		)
		Parallel(dy, dh).start()
	
	def pinPieces(self):
		for i in range(8):
			for j in range(8):
				if self.pieces[i,j]:
					self.pieces[i,j].obj.wrtReparentTo(self.squares[i,j])
	
	def unpinPieces(self):
		for i in range(8):
			for j in range(8):
				if self.pieces[i,j]:
					self.pieces[i,j].wrtReparentTo(self.render)
	...
	def inCheck(self, color):
		#find the king
		kingPlace = [p for p in self.pieces if self.pieces[p] and self.pieces[p].model == "models/king"][0]
		for p in self.pieces:
			if self.pieces[p] and self.pieces[p].isValidMove(kingPlace, self.pieces):
				return True
		return False
...
class Pawn(Piece):
	...
	def visibleSquares(self, pieces):
		#pawns have different visibility rules; namely they can *always* see one in front of them
		x,y = self.square
		return Piece.visibleSquares(self, pieces) | set([(x, y+self.dir)])
...
def selfTest():
	p = Pawn((1,1), PIECEWHITE)
	p.haveMoved = True
	print p.visibleSquares(defaultdict(lambda: None))

if len(sys.argv) == 1:
	print "Usage: %s -s|-c 0.0.0.0" % sys.argv[0]
	sys.exit(-1)
else:
	if sys.argv[1] == '-c':
		mode = CLIENT
		...
		mode = SERVER
if mode == CLIENT: w = World(mode=mode, ip=addr)
else: w = World(mode=mode)
run()